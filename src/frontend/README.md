...
SaveShell 进入存档先 POST content 再 GET 详情，同步固定目录、补缺失人物与已识别的旧模板姓名，保留定义 ID 和存档历史，不发书或重置进度；旧阅读成长按历史回执和最新额度快照补齐正差额，返回本次实际补发的十项 ReadingReward，非零时展示到账提示。相同配置不重复核算，成长配置变化后由后端处理已领额度；历史数据不足返回 409，进入流程保持错误状态，不绕过补齐直接开放写操作。状态以服务端为准：玩家 READY 考试优先，COMPLETED 进入结果；写请求结束后重新判断，地点记录不能覆盖业务状态。考试通过与否直接读 `COMPLETED_PASS/COMPLETED_FAIL`。
...
书目与行囊书籍共用 `BookCard`，按稳定 `equipmentCode/bookCode` 对应服务端书目。卡片显示 `completed/readable/blockedReasons`、当前进度与 `readingReward`；“条件不足”不提供阅读或写体会导航，“已完成”禁用后续阅读。可读书只在允许的场景直接执行，其他地点提供导航。书籍信息缺失时提示刷新，不伪装成可读。普通阅读和体会的实际成长来自 `changes.readingRewardGain`，旧回执缺字段不补造数值。
...
- 刷新后按 ID 重读对话；体会题只恢复同回合且对应书籍仍可写体会的缓存。换浏览器或清空本地存储后，当前没有查询全部未结束对话的接口，完整游戏事实及记忆正文仍在后端。
