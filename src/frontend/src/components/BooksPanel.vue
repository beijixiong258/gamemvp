...
<template>
  ...
  <p v-if="game.question" class="inline-note">《{{ game.question.bookName }}》的体会题已经备好。<button class="text-button" @click="emit('reading')">继续作答</button></p>
  ...
  <article v-for="book in books" :key="book.bookCode" class="book-card">
    ...
    <div class="book-body"><div class="book-title"><h3>{{ book.bookName }}</h3><span class="tag" :style="{ borderColor: book.rarityColor }">{{ book.rarityName }}</span><span v-if="book.completed" class="tag success">已掌握</span></div>
      ...
      <div class="button-row">
        ...
        <template v-else>
          ...
          <button v-if="book.playerReadingEnabled && allows(location, 'READ_BOOK_PLAYER')" :disabled="!game.canWrite || game.sick || !book.readable || book.completed" @click="reading(book)">以身入局 · 写体会</button>
          <button v-else-if="book.playerReadingEnabled && !book.completed" @click="emit('go', 'classroom')">去讲堂写体会 →</button>
        </template>
      </div>
    </div>
  </article>
